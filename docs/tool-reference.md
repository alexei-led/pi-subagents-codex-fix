...
| `executionLifetime` | `{ mode: "unbounded" }` or `{ mode: "bounded", timeoutMs }` | omitted | Explicit run lifetime. `unbounded` removes inherited run, tool, and worktree-hook deadlines; an explicit call-level `toolTimeoutMs` still applies. `bounded` uses the supplied positive integer deadline. Do not combine this field with `timeoutMs` or `maxRuntimeMs`; omission keeps the existing timeout defaults. The effective lifetime is preserved in async status and recovery records. |
| `timeoutMs` / `maxRuntimeMs` | number | config `timeoutMs`, else 30 min foreground / single-agent async | Optional legacy run-level max runtime in milliseconds. When omitted, the global [`timeoutMs`](configuration.md#timeoutms) config provides the default; absent that, foreground and plain single-agent async runs fall back to 30 minutes, while composite async runs (chains, parallel tasks, workflows) stay unbounded at the top level. Expiration of this run-level deadline is terminal. Use `executionLifetime` instead when the caller must explicitly select bounded or unbounded execution. |
