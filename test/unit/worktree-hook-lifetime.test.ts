...
it("keeps an explicitly unbounded real setup hook alive beyond the default thirty seconds", { skip: process.platform === "win32", timeout: 30_000 }, async t => {
  ...
  const git = (...args: string[]) => execFileSync("git", ["-c", "core.hooksPath=/dev/null", "-c", "user.name=Test", "-c", "user.email=test@example.invalid", ...args], { cwd: repo, env, stdio: "ignore" });
  git("init", "--quiet"); git("commit", "--quiet", "--allow-empty", "-m", "base");
