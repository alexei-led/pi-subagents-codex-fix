...
describe("native subagent fleet", () => {
	...
	it("restores current-session completed foreground history from the compact index", () => {
		...
		try {
			...
			assert.deepEqual(restored.foregroundRuns?.get("restored")?.children[0]?.resumeContract, { executionOwnership: { mode: "kernel" }, executionLifetime: { mode: "unbounded" }, outputSchema: { type: "object" }, agentContract: { version: 1 }, acceptance: false, output: false, outputMode: "inline" });
