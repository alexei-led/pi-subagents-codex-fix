...
describe("native durable rejected admission", () => {
  ...
  it("proves unknown-agent rejection before dispatch across cancellation, restart, and concurrent replay", { timeout: 30_000 }, async t => {
    ...
    t.after(() => rpc.dispose());
    let sequence = 0;
    const params = { operationId: "missing-agent-operation", digest: "missing-agent-digest", agent: "not-installed", task: "Do not dispatch", async: true, executionOwnership: { mode: "kernel" }, executionLifetime: { mode: "unbounded" } };
